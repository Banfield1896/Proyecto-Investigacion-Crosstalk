function out = testbench_XTC05
% TESTBENCH_XTC05 Exercise audio plugin class
% to check for violations of plugin constraints and other errors.
%
% OUT = TESTBENCH_XTC05 Return the output data from the
...
%
%   codegen testbench_XTC05    % Create the mex function
%   testbench_XTC05_mex        % Run the mex function
%
...
% validateAudioPlugin.
%
% Automatically generated by validateAudioPlugin 20-Aug-2025 16:26:34
...
for sampleRate = sampleRates
    ...
    for i = 1:numel(frameSizes)
        ...
        val = fromNormalizedloadIR(mod(floor((i-1)./81),3)/2);
        plugin.loadIR = val;
        paramState.loadIR = val;
        checkForTampering(plugin, paramState, sampleRate, ...
            'Setting parameter ''loadIR''');
...
if ~isequal(paramState.loadIR, plugin.loadIR)
    error('ValidateAudioPlugin:ParamChanged', ...
        '%s changed parameter ''loadIR'' from %g to %g.', ...
        cause, paramState.loadIR, plugin.loadIR);
end
...
paramState.loadIR = plugin.loadIR;
end
...
function val = fromNormalizedloadIR(normval)
