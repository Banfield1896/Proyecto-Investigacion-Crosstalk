classdef CrossConvolverStereo < audioPlugin & matlab.System & ...
        matlab.system.mixin.Propagates
    properties(Nontunable)
        % ImpulseResponseDirect
        % Specify the direct impulse responses (hL, hR) from a stereo file.
        ImpulseResponseDirect = audioread('response_st.wav').'; % Expects 2-channel: [hL; hR]
        ...
        ImpulseResponseCrossLR = audioread('responseLR.wav').'; % Expects 1-channel: hLR

        % ImpulseResponseCrossRL
        % Specify the cross impulse response for Right output from Left input (hRL).
        % Expected to be a mono WAV file.
        ImpulseResponseCrossRL = audioread('responseRL.wav').'; % Expects 1-channel: hRL

        % PartitionSize Length of each partition of the impulse reponse
        PartitionSize = 2048; %1024;
    end
    ...
    properties (Constant)
        PluginInterface = audioPluginInterface(...
            ...
            'PluginName','CrossConvolverStereo'...
            );
    end
